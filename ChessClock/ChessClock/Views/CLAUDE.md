...
**GoldRingLayerView.swift** — CALayer-based minute ring (Sprint 4N, optimized 4P). `NSViewRepresentable` with a `CALayer` whose `contents` is an IOSurface rendered by the GPU (zero-copy, no CPU readback). `GoldNoiseRenderer` renders at half-res (150×150) via Metal compute shader at 10 FPS, ring-masked by even-odd `CAShapeLayer`. Progress mask (pie wedge) on the gold container, updated 1/sec with 0.3s ease. Accepts `minute: Int`, `second: Int`, and `isActive: Bool`. When `isActive` is false (popover not visible), the noise timer is invalidated — zero CPU/GPU. When `isActive` returns to true, the timer restarts. Reduce motion: single static frame, no timer regardless of `isActive`.

**GoldNoiseRenderer.swift** — Metal compute pipeline manager (rewritten Sprint 4P). Failable `init?(width:height:)` creates `MTLDevice`, `MTLCommandQueue`, `MTLComputePipelineState` from `goldNoise` kernel, and pre-allocates 2 `IOSurface` + 2 `MTLTexture` pairs (double-buffer, `.bgra8Unorm`). `renderFrame(completion: @escaping (IOSurface) -> Void)` dispatches the compute kernel to the current texture and calls completion asynchronously via `addCompletedHandler` — main thread is never blocked. Caller sets `CALayer.contents = ioSurface` directly (zero-copy). Tunable `scale` (blob size) and `speed` (flow rate) properties.
