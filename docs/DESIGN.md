...
**Ring animation — Metal compute shader architecture (Sprint 4N):**

The ring texture is generated by a Metal compute shader (`GoldNoiseShader.metal`) that produces 3D FBM simplex noise mapped through a 5-tone gold color ramp. `GoldNoiseRenderer` manages the Metal pipeline and renders at half resolution (150×150), upscaled by `CALayer.contentsGravity = .resize`. A 10 FPS Timer drives frame updates. The ring looks like slowly flowing liquid gold.

**Design principle (Sprint 4N):** The CAGradientLayer + locations drift approach (Sprint 4F) produced a "chuggy" pattern — linear interpolation between fixed gradient stops is not organic. Real noise requires GPU computation. A Metal compute kernel generates true simplex noise at minimal CPU cost (~0.2ms/frame for dispatch + readback at half-res).
...
NSViewRepresentable ("GoldRingLayerView")
  └─ NSView (wantsLayer = true, isFlipped = true)
      ...
      │   ├─ noiseLayer: CALayer               — Metal noise texture, ring-masked, 10 FPS
      │   │   └─ GoldNoiseRenderer             — Metal compute pipeline, half-res (150×150)
...
- **Noise texture (liquid gold):** `GoldNoiseShader.metal` computes 3D simplex noise (Gustavson/McEwan) with 2-octave FBM (persistence 0.5, lacunarity 2.0). The third dimension is time, so the pattern evolves smoothly. Output is mapped through a 5-tone gold color ramp (deep → cool → primary → warm → light) via smoothstep segments. `GoldNoiseRenderer` renders at 150×150 and converts the `MTLTexture` to `CGImage` via pixel readback. A 10 FPS Timer assigns the image to `noiseLayer.contents`. Noise parameters: `scale = 0.012` (large blobs), `speed = 0.22` (moderate flow).
...
**What was removed (Sprint 4N):** `CAGradientLayer`, 17-color gradient arrays, `baseLocations`/`driftedLocations`, `CABasicAnimation` color drift. Prior to that (Sprint 4F): gradient rotation, glowing tip, breathing pulse, spring physics.

**Performance target:** <0.5% CPU sustained. Metal compute runs on GPU (~0.05ms). Texture readback + layer update ~0.15ms/frame × 10 FPS = ~1.5ms/sec total ≈ 0.15% CPU. App-side: ~0.01ms once per second for wedge path update.
...
1. **Use Core Animation / Metal for continuous animation, not SwiftUI.** SwiftUI's `AngularGradient`, `TimelineView`, and shape-based animation all run on the app's CPU (10-15% for the ring). For continuously animated elements, use `CALayer` via `NSViewRepresentable`. For procedural textures (noise, gradients), use Metal compute shaders rendered to `CALayer.contents` at a controlled frame rate. The minute ring is the canonical example: `GoldNoiseShader.metal` generates simplex noise on the GPU, `GoldNoiseRenderer` renders at 150×150 (half-res) at 10 FPS, and the `CALayer` displays the upscaled result — achieving flowing liquid gold at <0.2% CPU. Reserve SwiftUI `.animation(.linear, value:)` for discrete state transitions (progress advance, face changes) that happen infrequently.
